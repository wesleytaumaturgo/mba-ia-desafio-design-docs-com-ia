...
| --- | --- | --- |
| 2026-08-19 | **FDD-1..FDD-7 implementados** em `scripts/verify.sh` v5, com três desvios em relação às linhas 23–28 desta matriz | (a) O piso de palavras do FDD-1 sobe de **120 para 1500**: 120 era um piso genérico derivado de "12 seções × 10 palavras", e o bloco 6 fixou 1500 — o documento entregue tem 6078, então o piso alto continua sendo piso, não teto disfarçado. (b) O FDD-3 deixa de contar ocorrências de `FDD-CONTRATO-NN` e passa a exigir, **por bloco** `### MÉTODO /path`, dois fences ```json e uma linha `**Status:** NNN` (C-4); contar IDs mediria rastreabilidade, não contrato. (c) O FDD-5 cruza contra `git ls-files` e não contra `.planning/00-inventario-paths.txt`, seguindo a retificação de 2026-08-18 desta mesma tabela. Acrescentado **FDD-7**, que não está na tabela de critérios: é o invariante mecânico de D-10 (`.planning/03-design.md` §10) — nenhuma linha do FDD pode afirmar que a coluna do schema se chama `total_cents`, `order_number`, `from_status`, `to_status` ou `stock_quantity`. Provado por N1..N8 em `.planning/06-teste-negativo.md`. **Não houve MAN-03**: o FDD-7 saiu discriminante na primeira tentativa, sem falso positivo sobre o documento limpo. |
| 2026-08-19 | Leitura registrada do campo medido pelo **FDD-4** | C-5 (`.planning/03-design.md` §7) diz "primeira coluna da matriz casa `WEBHOOK_[A-Z_]{3,}`", mas o formato de tabela fixado pelo bloco 6 é `\| ID (FDD-ERR-NN) \| Código \| HTTP \| …` — a primeira célula é o ID de rastreabilidade exigido por C-2, e o código é a segunda. O check mede a **célula de código**, e a segunda perna (zero token `SCREAMING_SNAKE` sem o prefixo em toda a seção) é a que implementa DEC-13 de fato: ela pega o código errado esteja ele em linha de tabela ou em prosa. |
